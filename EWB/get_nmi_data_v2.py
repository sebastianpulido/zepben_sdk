...
async def connect():
    channel = connect_with_token(host=c["host"], rpc_port=c["rpc_port"], access_token=c["access_token"], ca_filename=c["ca_filename"])
...
class Node(object):
    ...
    cross_reference_type_asset_id: str
        ...
        for equipment in site.equipment:
            ...
            if isinstance(equipment, Switch):
                ...
                for conducting_equipment in lvf.equipment:
                    try:
                        asset_id = str(list(conducting_equipment.names)[0].name)
                    except:
                        asset_id = "<None asset id>"
                    ...
                    if upstream_equip:
                        ...
                        asset_id_upstream = "<None asset id upstream>"
                        try:
                            for name in upstream_equip.get_names("GIS"):
                                upstream_node = name.name
                                asset_id_upstream = name.name
                        except KeyError as k:
                    ...
                    network_nodes.append(
                        Node(
                            ...
                            asset_id_upstream=asset_id_upstream,
                            conductor_type=conductor_code,
                            cross_reference_type=node_id,
                            cross_reference_type_asset_id=asset_id,
