...
class asset_data:
        ...
        for fdr in network_hierarchy.feeders.values():
            if not found:
                ...
                try:
                    ...
                    print(f"exception: {e} - {e.__traceback__}")
            else:
                break
    ...
    def get_asset_data_with_Feeder(self, feeder, mrid, type):
        ...
        network_client = SyncNetworkConsumerClient(channel=channel)
        network_client.get_equipment_container(feeder, include_energized_containers=IncludedEnergizedContainers.INCLUDE_ENERGIZED_LV_FEEDERS).throw_on_error()
        network: NetworkService = network_client.service
        ...
        try:
            ...
            for obj in objects:
                if str(clss(obj).mrid).__contains__(mrid):
                    line = f"'{feeder}';'{type}';'{clss(obj).mrid}';'{clss(obj).in_service}';'{clss(obj).is_normally_open()}';'{clss(obj).is_open()}';'{list(clss(obj).names)}';'{clss(obj).name if clss(obj).name is not None else ""}'"
                    self.write_csv_file(line, False)
    ...
    def load_csv_file(self):
        ...
        with open("./EWB/inputs/assets.csv", newline="", encoding="utf-8") as csvfile:
            ...
            for row in csv_reader:
                ...
                gis = row[1]
                mrid = row[2]
                print(f"type:{type} mrid: {mrid}")
                self.get_asset_data_loopingFeeders(mrid, type)
    ...
    def load_csv_file_with_feeder(self):
        ...
        with open("./EWB/inputs/assets.csv", newline="", encoding="utf-8") as csvfile:
            ...
            for row in csv_reader:
                ...
                type = row[1]
                mrid = row[2]
                print(f"feeder:{feeder} - type:{type} - mrid:{mrid}")
                self.get_asset_data_with_Feeder(feeder, mrid, type)

    def write_csv_file(self, line, write_header):
        header = "feeder,asset,mrid,in service,is normally open,is open,list of names,name"
