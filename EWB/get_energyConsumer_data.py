    ...
    def get_energyConsumer_recon_data_by_feeder_name(self, feeders_group_name):
        ...
        for ec in network.objects(EnergyConsumer):
            ...
            if ec.__str__().__contains__("public_light"):
                ...
                _type_of_asset = "supply_point"
            line = f"'{feeders_group_name}';'{ec.mrid}';'{_type_of_asset}';'{ec.__str__()}';'{ec.in_service}';'{list(ec.normal_feeders)}';'{list(ec.names)[0].name}';'{ec.name}';'{list(ec.current_containers)}';'{list(ec.containers)}"
            cleaned_row = [value.strip("'") for value in line.split("';'")]
            create_csv(f"./{filename}", *cleaned_row)
...
data.get_energyConsumer_recon_data_by_feeder_name("BKN")
data.get_energyConsumer_recon_data_by_feeder_name("BLT")
