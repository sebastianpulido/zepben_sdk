...
from zepben.evolve import PerLengthSequenceImpedance, Fuse, Conductor, Disconnector, PowerTransformer, Circuit, Loop, PowerSystemResource, ConnectivityNode, Terminal, Meter, ConductingEquipment, PowerTransformer, Breaker, EnergyConsumer, LvFeeder, AcLineSegment, connect_with_secret, TransformerFunctionKind, connected_equipment, UsagePoint, Equipment, Switch, Feeder, BaseService, GeographicalRegion, BusbarSection, Substation


class disconnector_data:
    def __init__(self):
        self.name = self.__class__.__name__
        now = datetime.datetime.now().strftime("%d%m%Y")
        self.basepath = "./EWB/outputs"
        self.data_path = f"{self.basepath}/{self.name}_{now}.csv"
        self.connections_path = f"{self.basepath}/{self.name}_connections_{now}.csv"
...
data.get_disconnector_data_allfeeders("PTN")